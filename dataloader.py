...
class Load_Dataset(Dataset):
    def __init__(self, dataset, normalize):
        ...
        if len(X_train.shape) < 3:
            ...
            # 如果"samples"的维度小于3，则使用unsqueeze()方法在第二维度中添加一个新维度
            # 这是因为"samples"张量至少必须具有三个维度，以适应PyTorch中的CNN模型

        if X_train.shape.index(min(X_train.shape[1], X_train.shape[2])) != 1:  # make sure the Channels in second dim
            X_train = X_train.permute(0, 2, 1)
            # 如果第二个和第三个维度中的较小值不在第二个维度中，则使用permute()方法将第二个维度和第三个维度交换
            # 这个操作确保输入数据中通道位于第二个维度中

        self.x_data = X_train
        self.y_data = y_train

        self.num_channels = X_train.shape[1]
        # 将预处理后的数据样本，标签，通道数存储在类实例的属性中
        ...
        if normalize:
            ...
            # 这个变换对象将标准化输入数据的每个通道，使其均值为0，标准差为1
...
def data_generator(dataset_name, domain_id, normalize=True, batchsize=64):
    ...
    # Loading datasets
    train_dataset = Load_Dataset(train_dataset, normalize)
    test_dataset = Load_Dataset(test_dataset, normalize)
